...
export default function GlobalLoader() {
    ...
    const generateRandomAudioBars = () => {
        return Array.from({ length: 11 }, () => Math.floor(Math.random() * 70) + 30);
    };
                ...
                <>
                    ...
                    {progress < 100 && (
                        ...
                        >
                            ...
                            <div className="backdrop-blur-lg px-4 sm:px-8 py-6 rounded-2xl border border-white/10 shadow-[0_0_30px_rgba(42,33,81,0.5)] flex flex-col items-center justify-center gap-4 max-w-sm mx-auto">
                                ...
                                <motion.div 
                                    className="mt-2 px-4 py-1 rounded-full text-white/80 text-xs border border-white/10 text-center"
                                    initial={{ opacity: 0 }}
                                    animate={{ opacity: 1 }}
                            ...
                            <AnimatePresence>
                                {loading && (
                                    <div className="absolute" style={{ width: '300px', height: '250px', left: '50%', top: '50%', transform: 'translate(-50%, -50%)' }}>
                                        <motion.div
                                            className="absolute -top-12 -left-8 text-[#20DDBB] text-2xl hidden sm:block"
                                        ...
                                        <motion.div
                                            className="absolute -bottom-10 left-10 text-[#8A2BE2] text-3xl hidden sm:block"
                                            variants={noteVariants}
                                            initial="hidden"
                                            animate="visible"
                                            exit="exit"
                                        ...
                                        <motion.div
                                            className="absolute -top-8 right-2 text-[#6A5ACD] text-xl hidden sm:block"
                                            variants={noteVariants}
                                            initial="hidden"
                                            animate="visible"
                                            exit="exit"
                                        ...
                                        <motion.div
                                            className="absolute bottom-2 -right-10 text-[#20DDBB] text-2xl hidden sm:block"
                                            variants={noteVariants}
                                            initial="hidden"
                                            animate="visible"
                                            exit="exit"
