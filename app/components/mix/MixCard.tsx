      ...
      <Link href={`/mix/${mix.id}`} className="block h-full">
        ...
        <div className="p-5">
          <div className="flex items-center mb-3">
            ...
            <span className="text-sm font-medium text-white/90">{mix.profile.name}</span>
          </div>
          
          <h3 className="font-bold text-xl mb-2 line-clamp-1 text-white group-hover:text-purple-300 transition-colors">{mix.title}</h3>
          <p className="text-gray-300 text-sm mb-3 line-clamp-2">{mix.description}</p>
          
          <div className="flex justify-between items-center text-sm text-gray-300">
            <div className="flex items-center space-x-3">
              <span className="bg-gradient-to-r from-purple-600/80 to-indigo-600/80 px-3 py-1 rounded-full text-xs text-white font-medium">{mix.genre}</span>
              <span className="text-gray-400">{formatDistanceToNow(new Date(mix.created_at), { addSuffix: true })}</span>
            </div>
