...
import { PlayIcon, PauseIcon, ChatBubbleLeftIcon, ShareIcon } from '@heroicons/react/24/solid';
...
function useComments(mixId: string) {
  ...
  const addComment = (newComment: MixComment) => {
    ...
    const existingCommentIndex = comments.findIndex(comment => comment.id === newComment.id);
  ...
  return { comments, isLoading, error, totalComments, fetchComments, addComment };
}
...
export default function MixDetailPage({ mix }: MixDetailPageProps) {
  ...
  const { comments, isLoading: isLoadingComments, error: commentsError, totalComments, fetchComments, addComment } = useComments(mix.id);
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        <div className="p-6">
          ...
          <div className="flex items-center mb-4">
            <Link href={`/profile/${mix.user_id}`} className="flex items-center">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">{formatDistanceToNow(new Date(mix.created_at), { addSuffix: true })}</p>
              </div>
            </Link>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            <span className="bg-gray-100 px-3 py-1 rounded-full text-sm">{mix.genre}</span>
          ...
          <div id="comments-section">
            <h2 className="text-xl font-bold mb-4">Comments ({totalComments})</h2>
            ...
            {user ? (
              <form onSubmit={handleSubmitComment} className="mb-6">
                <div className="flex items-start gap-3">
                  <div className="relative h-10 w-10 rounded-full overflow-hidden flex-shrink-0">
                    <Image 
                      src={user.image ? getProfileImageUrl(user.image) : '/images/default-avatar.png'} 
                      alt={user.name || 'User'} 
                      fill 
                      className="object-cover"
                    />
              ...
              <div className="bg-gray-50 p-4 rounded-lg mb-6">
                <p className="text-center text-gray-600">
                  <Link href="/login" className="text-primary font-medium hover:underline">Sign in</Link> to add a comment
                </p>
              </div>
            )}
            ...
            {isLoadingComments ? (
              ...
              <div className="space-y-6">
                {comments.map((comment) => (
                  ...
                  >
                    <Link href={`/profile/${comment.user_id}`} className="relative h-10 w-10 rounded-full overflow-hidden flex-shrink-0">
                      <Image 
                        src={comment.profile?.image ? getProfileImageUrl(comment.profile.image) : '/images/default-avatar.png'} 
