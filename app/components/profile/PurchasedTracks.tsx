...
export default function PurchasedTracks() {
  ...
  // Показываем пустое состояние, когда загрузка завершена, но покупок нет
  if (!loading && purchases.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] text-center">
        <BsCart className="w-16 h-16 text-[#20DDBB]/30 mb-4" />
        <p className="text-white text-lg mb-2">No purchases yet</p>
        <p className="text-gray-400">Your purchased tracks will appear here</p>
      </div>
    );
  }
