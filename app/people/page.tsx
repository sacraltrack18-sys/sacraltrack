...
import UniversalSkeleton from "@/app/components/ui/UniversalSkeleton";
import SimpleSidebarLoading from "@/app/components/ui/SimpleSidebarLoading";

// Используем внутренний компонент UserCard (определен ниже)
...
export default function People() {
  ...
  // Рейтинг пользователя (первый вариант - переименован)
  const handleRateUserFirst = async (userId: string, rating: number) => {
    if (!user?.user?.id) {
        ...
        <div className={styles.pageContent}>
          <div className="space-y-4">
            ...
            <div className={styles.gridLayout}>
              {[...Array(6)].map((_, i) => (
                <UniversalSkeleton key={i} variant="user" />
              ))}
            </div>
          </div>
        </div>
  ...
  return (
    <>
      <Head>
        <title>People - Discover Artists & Connect | Sacral Track</title>
        <meta name="description" content="Discover talented artists, musicians, and creators. Connect with the community, rate profiles, and build your network on Sacral Track." />
        <meta name="keywords" content="artists, musicians, creators, music community, social network, discover talent, rate artists, music platform" />
        <meta property="og:title" content="People - Discover Artists & Connect | Sacral Track" />
        <meta property="og:description" content="Discover talented artists, musicians, and creators. Connect with the community and build your network." />
        <meta property="og:type" content="website" />
        <meta property="og:url" content="https://sacraltrack.com/people" />
                    ...
                    <div className={styles.gridLayout}>
                      {isLoading
                        ? Array.from({ length: INITIAL_PAGE_SIZE }).map(
                            (_, index) => (
                              <div key={index}>
                                <UniversalSkeleton variant="user" />
                              </div>
                            ),
                          )
